<template>
  <div class="hidden lg:flex lg:flex-grow justify-center items-center xl:text-xl mt-12 space-x-12 font-poppins"
    ...
    <NuxtLink to="/" v-if="isNavFixed">
      <img src="~/assets/img/logo/Original_Logo_Symbol-removebg-preview.png" alt="Menuiserie Gibilaro logo"
        class="logo-nav-fixed w-20 absolute bottom-2 lg:left-10 xl:left-20" />
    </NuxtLink>
    ...
    <div class="relative group" @mouseover="showSubMenu = true" @mouseleave="showSubMenu = false">
      <NuxtLink to="/realisations"
        class="text-white transition-all duration-300 hover:text-yellow opacity-90 hover:opacity-100 group"
      ...
      <div v-show="showSubMenu" class="subMenu absolute z-50 mt-0 top-full left-0 bg-black text-white p-2 shadow-md"
        ...
        <NuxtLink to="/realisations#escaliers"
        ...
        <NuxtLink to="/realisations#portes"
          class="block mb-4 pb-2 pl-2 border-b transition-all duration-300 hover:text-yellow">
